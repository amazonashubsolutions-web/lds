...
as $$
  select ((p_travel_date::timestamp + p_embark_time) at time zone 'America/Bogota') - interval '24 hours';
$$;
...
set expires_at = case
  when status = 'reserved'
    then public.calculate_reservation_expires_at(travel_date, embark_time)
  else null
end
...
begin
  ...
  if p_status = 'reserved' then
    v_expires_at := public.calculate_reservation_expires_at(
      p_travel_date,
      p_embark_time
    );
