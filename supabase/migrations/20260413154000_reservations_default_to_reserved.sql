...
set new_status = 'reserved',
    reason = case
      when reason = 'Reserva creada desde el detalle del producto. Pendiente de checkout.'
        then 'Reserva creada desde el detalle del producto.'
      else reason
    end
where new_status = 'payment_pending';
...
declare
  ...
  v_effective_status varchar := coalesce(nullif(p_status, ''), 'reserved');
begin
