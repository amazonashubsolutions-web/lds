...
function formatMonthDayLabel(monthDay) {
  ...
  if (!Number.isInteger(monthIndex) || monthIndex < 0 || monthIndex >= MONTH_LABELS.length || !Number.isInteger(dayNumber) || dayNumber <= 0) {
    return "";
  }
...
function formatSeasonPeriodLabel(period = {}) {
  ...
  const rangeLabel = startLabel && endLabel ? `del ${startLabel} al ${endLabel}` : "";
  if (!label) return rangeLabel || "Periodo por definir";
  return label.toLowerCase().includes("del ") || label.toLowerCase().includes(startLabel) ? label : `${label}: ${rangeLabel}`;
}
...
function buildAvailableCouponDescription(coupon) {
  ...
  return !description ? `sobre el ${discountTargetLabel}` : `${description}, sobre el ${discountTargetLabel}`;
}
...
function getClampedTravelDateValue(value, minDate, maxDate) {
  ...
  if (parsedDate < minDate) return formatDateForInput(minDate);
  if (parsedDate > maxDate) return formatDateForInput(maxDate);
  return value;
}
...
function getActiveSeasonKey(travelDate, seasons) {
  if (!travelDate || !seasons?.high?.periods?.length) return null;
...
export default function ProductTransportBookingCard({
  ...
  const activePricingItem = activeSeason?.individual?.[0] ?? pricingDetails?.individual?.[0] ?? { price: booking.price };
  ...
  const fixedChargesTotal = additionalCharges.reduce(
    (sum, item) => item.type === "fixed" ? sum + parseBookingPrice(item.value) : sum,
    0
  );
  
  const estimatedTotal = unitPrice + fixedChargesTotal;
  
  // Fake single passenger array just to trick the coupon engine properly
  ...
  const activeAppliedCoupon = appliedCouponEvaluation?.isEligible ? appliedCouponEvaluation : null;
  const totalAfterDiscount = activeAppliedCoupon?.totalAfterDiscount ?? estimatedTotal;
  ...
  useEffect(() => {
    if (!isPricingModalOpen) return undefined;
    const previousOverflow = document.body.style.overflow;
    const handleKeyDown = (event) => { if (event.key === "Escape") setIsPricingModalOpen(false); };
  ...
  function handleCouponCodeChange(event) {
    ...
    if (appliedCouponCode && normalizedValue.trim() !== appliedCouponCode) setAppliedCouponCode("");
  ...
  function handleApplyCoupon() {
    ...
    if (!normalizedCode) return showCouponError("Digita un codigo de cupon antes de aplicarlo.");
    if (!travelDate) return showCouponError("Selecciona primero la fecha de viaje para validar el cupon.");

    const selectedCoupon = findProductCouponByCode(normalizedCode, booking.productId);
    if (!selectedCoupon) {
      setAppliedCouponCode("");
    ...
    if (!evaluation.isEligible) {
      setAppliedCouponCode("");
      return showCouponError(evaluation.reason || "La reserva no cumple las condiciones del cupon.");
    }

    setAppliedCouponCode(normalizedCode);
    setCouponCode(normalizedCode);
    setCouponFeedback({ tone: "success", message: `${selectedCoupon.code} aplicado correctamente.` });
  }
    ...
    <>
      <aside className="detalle-producto-booking-card" style={{ ...categoryThemeStyle, background: "#ffffff", overflow: "hidden", borderRadius: "12px", boxShadow: `0 10px 25px ${theme.shadow}`, padding: "0" }}>
        ...
        <div style={{ background: `linear-gradient(135deg, ${theme.primary} 0%, ${theme.secondary} 100%)`, padding: "1.5rem 1.5rem 1.25rem 1.5rem", textAlign: "center" }}>
          ...
          <div className="detalle-producto-booking-top" style={{ margin: "0", padding: "0", background: "transparent", border: "none", alignItems: "center", display: "flex", flexDirection: "column" }}>
            <strong style={{ lineHeight: "1", color: "#ffffff", fontSize: "2rem" }}>{formatBookingPrice(startingPrice)}</strong>
            <small style={{ margin: "0.25rem 0 0", color: "rgba(255, 255, 255, 0.85)", fontSize: "0.9rem" }}>{booking.unitLabel ?? "por trayecto"}</small>
            {pricingDetails && (
              <button
                type="button"
                className="detalle-producto-booking-info-link"
                onClick={() => setIsPricingModalOpen(true)}
                style={{ marginTop: "0.5rem", color: "#ffffff", textDecoration: "underline", background: "transparent", border: "none", cursor: "pointer", fontSize: "0.85rem", opacity: "0.9" }}
              >
        ...
        <div style={{ padding: "1.5rem", background: "#ffffff" }}>
          <div className="detalle-producto-booking-form" style={{ marginTop: "0", padding: "0" }}>
            <label>
              <span style={{ fontWeight: "600", marginBottom: "0.5rem", display: "block", color: "#374151" }}>Fecha de Servicio</span>
              <input
                type="date"
                value={travelDate}
          ...
          <div className="detalle-producto-booking-coupons" style={{ marginTop: "1rem" }}>
            <div className="detalle-producto-booking-coupons-head">
              <span style={{ color: "#4b5563" }}>Cupones disponibles</span>
            </div>

            {!travelDate ? (
              <p className="detalle-producto-booking-coupons-empty" style={{ color: "#6b7280" }}>
                Ingresa una fecha para verificar promociones.
              </p>
            ) : availableCoupons.length ? (
              <div className="detalle-producto-booking-coupon-list" style={{ marginTop: "0.5rem" }}>
                {availableCoupons.map((coupon) => (
                  <article className="detalle-producto-booking-coupon-card" key={coupon.id} style={{ border: "1px solid #e5e7eb", borderRadius: "6px", padding: "0.5rem" }}>
                    <strong style={{ color: "#1f2937" }}>{coupon.code}</strong>
                    <p style={{ color: "#4b5563", fontSize: "0.85rem", margin: "0" }}>{buildAvailableCouponDescription(coupon)}</p>
                  </article>
                ))}
              </div>
            ) : (
               <p className="detalle-producto-booking-coupons-empty" style={{ color: "#6b7280" }}>No hay cupones activos.</p>
            )}
          </div>
          ...
          {isHighSeasonActive ? (
            <div className="detalle-producto-booking-inline-alert" style={{ marginTop: "1rem" }}>
              <div className="detalle-producto-booking-season-alert" style={{ background: theme.softAlt, color: theme.accent, borderColor: theme.surfaceStrong }}>
                ...
                <span className="material-icons-outlined detalle-producto-booking-alert-icon" aria-hidden="true">trending_up</span>
              </div>
            </div>
          ) : null}

          <div className="detalle-producto-booking-breakdown" style={{ marginTop: "1.5rem", borderTop: "2px dashed #e5e7eb", paddingTop: "1rem" }}>
            <div className="detalle-producto-booking-row" style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.5rem" }}>
              <span style={{ color: "#4b5563" }}>Valor Trayecto/Vehículo</span>
              <strong style={{ color: "#1f2937" }}>{formatBookingAmount(unitPrice)}</strong>
            </div>

            {additionalCharges.map((item) => (
              <div className="detalle-producto-booking-row" key={item.label} style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.5rem" }}>
                <span style={{ color: "#4b5563" }}>{item.label}</span>
                <strong style={{ color: "#1f2937" }}>{item.type === "fixed" ? formatBookingAmount(parseBookingPrice(item.value)) : item.value}</strong>
              </div>
            ))}
            ...
            {activeAppliedCoupon && (
              <>
                <div className="detalle-producto-booking-row detalle-producto-booking-row--discount" style={{ display: "flex", justifyContent: "space-between", color: "#059669", marginTop: "0.5rem" }}>
                  <span>Cupón {activeAppliedCoupon.coupon.code}</span>
                  <strong>{formatBookingDiscount(activeAppliedCoupon.discountAmount)}</strong>
                </div>
                <div className="detalle-producto-booking-row detalle-producto-booking-row--final" style={{ display: "flex", justifyContent: "space-between", fontSize: "1.2rem", fontWeight: "700", marginTop: "0.5rem", color: "#111827" }}>
          ...
          <div className="detalle-producto-booking-coupon-form" style={{ marginTop: "1rem" }}>
            <div style={{ display: "flex", gap: "0.5rem" }}>
              ...
              <button
                ...
                style={{ padding: "0.75rem 1rem", background: "#f3f4f6", borderRadius: "8px", border: "1px solid #d1d5db", fontWeight: "600", cursor: "pointer", color: "#374151" }}
              >
                Aplicar
              </button>
            </div>
            {couponFeedback && (
              <p className={`detalle-producto-booking-coupon-feedback detalle-producto-booking-coupon-feedback--${couponFeedback.tone}`} style={{ marginTop: "0.5rem", fontSize: "0.85rem", color: couponFeedback.tone === "error" ? "#dc2626" : "#059669" }}>
      ...
      {isPricingModalOpen && pricingDetails && createPortal(
        <div className="detalle-producto-booking-modal-backdrop" onClick={() => setIsPricingModalOpen(false)}>
          <div className="detalle-producto-booking-modal" onClick={e => e.stopPropagation()}>
            <button className="detalle-producto-booking-modal-close" onClick={() => setIsPricingModalOpen(false)}>&times;</button>
            <div className="detalle-producto-booking-modal-head">
               <h3 id="detalle-producto-booking-modal-title">Tarifas del Vehículo</h3>
            </div>
            ...
            {seasonalPricing && ["low", "high"].map((seasonKey) => {
              ...
              return (
                 <div className="detalle-producto-booking-modal-section" key={seasonKey}>
                   <h4>{season.title}</h4>
                   {season.note && <p className="detalle-producto-booking-modal-rule">{season.note}</p>}
                   {season.periods?.length > 0 && (
                     <div className="detalle-producto-booking-modal-rule">
                        <p>Fechas:</p>
                        <ul>{season.periods.map(p => <li key={p.id}>{formatSeasonPeriodLabel(p)}</li>)}</ul>
                     </div>
                   )}
                   <div style={{ padding: "1rem", background: "#f8f9fa", borderRadius: "8px", marginTop: "0.5rem", textAlign: "center", border: "1px solid #e5e7eb" }}>
                     <strong style={{ fontSize: "1.2rem", color: seasonKey === "high" ? "#c53030" : "#0f766e" }}>
                       {formatBookingPrice(season.individual[0]?.price || booking.price)}
                     </strong>
                     <p style={{ margin: "0.2rem 0 0", fontSize: "0.85rem", color: "#6b7280" }}>{booking.unitLabel ?? "por trayecto"}</p>
                   </div>
                 </div>
              );
            })}
