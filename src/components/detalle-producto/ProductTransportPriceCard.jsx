...
function formatSeasonPeriodLabel(period = {}) {
  ...
  const rangeLabel =
    startLabel && endLabel ? `${startLabel} a ${endLabel}` : "";
  ...
  if (
    ...
    normalizedLabel.includes(startLabel.toLowerCase()) ||
    normalizedLabel.includes(endLabel.toLowerCase())
  ) {
    return rangeLabel;
  }
...
function InlineActionButton({ label, icon, onClick, variant = "add" }) {
  return (
    <button
      type="button"
      className={`detalle-producto-inline-action detalle-producto-inline-action--${variant}`}
      onClick={onClick}
    >
      <span className="material-icons-outlined" aria-hidden="true">
        {icon}
    ...
    <div className="detalle-producto-booking-modal-grid">
      <article
        className={`detalle-producto-booking-modal-card${
          tone === "high" ? " detalle-producto-booking-modal-card--high" : ""
        }`}
      >
        <span>Tarifa Total por Vehículo</span>
        {isEditing ? (
          <PriceInputField
            label="Precio (Flat)"
            value={priceValue}
            onChange={(nextValue) => onPriceChange(nextValue)}
          />
        ) : (
          <strong>{formatBookingPrice(priceValue)}</strong>
        )}
      </article>
    </div>
  );
...
function HighSeasonPeriodsEditor({
  periods = [],
  isEditing = false,
  onPeriodChange = () => {},
  onAddPeriod = () => {},
  ...
  return (
    <section className="detalle-producto-price-card-periods">
      ...
      {periods.length > 0 ? (
        <div 
          className={isEditing ? "detalle-producto-price-card-period-list" : ""}
          style={!isEditing ? { display: "flex", flexWrap: "wrap", gap: "0.4rem" } : undefined}
        >
              ...
              <article className="detalle-producto-price-card-period" key={period.id}>
                <label className="detalle-producto-admin-edit-field detalle-producto-price-card-field">
                  ...
                  <input
                    ...
                    value={period.label ?? ""}
                    onChange={(event) =>
                      onPeriodChange(index, "label", event.target.value)
                    }
                ...
                <div className="detalle-producto-price-card-period-dates">
                  <label className="detalle-producto-admin-edit-field detalle-producto-price-card-field">
                    ...
                    <input
                      type="date"
                      value={getDateInputValue(period.startMonthDay)}
                      onChange={(event) =>
                        onPeriodChange(
                          index,
                          "startMonthDay",
                          getMonthDayFromInputValue(event.target.value),
                        )
                      }
                    />
                  </label>
                  ...
                  <label className="detalle-producto-admin-edit-field detalle-producto-price-card-field">
                    ...
                    <input
                      type="date"
                      value={getDateInputValue(period.endMonthDay)}
                      onChange={(event) =>
                        onPeriodChange(
                          index,
                          "endMonthDay",
        ...
        <p className="detalle-producto-create-empty">
          No hay periodos de temporada alta definidos todavia.
        </p>
      )}
    </section>
  );
...
export default function ProductTransportPriceCard({
  ...
  onBasePriceChange = () => {},
  onGridPriceChange = () => {},
  onHighSeasonPeriodChange = () => {},
  onAddHighSeasonPeriod = () => {},
  onRemoveHighSeasonPeriod = () => {},
  ...
  // En transporte, usamos individual[0] para almacenar la tarifa general
  const getTarifaVehiculo = (seasonKey) => {
    return seasonalPricing?.[seasonKey]?.individual?.[0]?.price ?? booking.price;
  };
    ...
    >
      <div className="detalle-producto-booking-top" style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "1.1rem" }}>
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          <span style={{ fontSize: "0.72rem", fontWeight: "800", textTransform: "uppercase", letterSpacing: "0.05em", color: "rgba(255, 255, 255, 0.85)" }}>Tarifa flat de Transporte</span>
          <div style={{ display: "grid", gap: "0.1rem" }}>
            <strong style={{ fontSize: "2.15rem", color: "#ffffff", lineHeight: "1" }}>{formatBookingPrice(booking.price)}</strong>
            <small style={{ fontSize: "0.68rem", color: "rgba(255, 255, 255, 0.85)", fontWeight: "600", paddingTop: "0.15rem" }}>Valor Base (Temporalidad baja)</small>
          </div>
        </div>

        <div style={{ display: "grid", gap: "0.4rem" }}>
          <span style={{ fontSize: "0.72rem", fontWeight: "800", textTransform: "uppercase", letterSpacing: "0.05em", color: "rgba(255, 255, 255, 0.85)" }}>FECHAS TEMPORADA ALTA</span>
          {seasonalPricing?.high?.periods?.length > 0 ? (
            ...
            <span style={{ fontSize: "0.68rem", color: "rgba(255, 255, 255, 0.85)" }}>Sin fechas aplicadas</span>
          )}
        </div>
      </div>

      <div className="detalle-producto-price-card-body">
        {isEditingEnabled && showEditToggle ? (
          <div className="detalle-producto-price-card-edit-head">
            <span>Edición de precio de vehículo</span>
            <PriceEditAction
              isActive={isPricingActive}
              onClick={() => onActivateBlock("pricing")}
            />
          </div>
        ) : null}
        ...
        {isPricingActive ? (
          <div className="detalle-producto-price-card-summary">
            <p>Tarifa base del servicio de transporte</p>
            <PriceInputField
              label="Precio base"
              value={booking.price}
              onChange={onBasePriceChange}
            />
            <span>Valor predeterminado aplicado</span>
          </div>
        ) : null}
        ...
        {seasonalPricing?.high && isPricingActive ? (
          <HighSeasonPeriodsEditor
            ...
            onPeriodChange={onHighSeasonPeriodChange}
            onAddPeriod={onAddHighSeasonPeriod}
            onRemovePeriod={onRemoveHighSeasonPeriod}
          />
        ) : null}
        ...
        {seasonalPricing ? (
          ["low", "high"].map((seasonKey) => {
            ...
            return (
              <section
                className="detalle-producto-price-card-section"
                key={seasonKey}
              >
                ...
                <FlatPriceEditor
                  ...
                  isEditing={isPricingActive}
                  priceValue={getTarifaVehiculo(seasonKey)}
                  onPriceChange={(nextValue) =>
                    onGridPriceChange(seasonKey, "individual", 0, nextValue) // Guardamos en el flag 0 el global
                  }
                />
              </section>
            );
          ...
          <section className="detalle-producto-price-card-section">
            <div className="detalle-producto-price-card-section-head">
            ...
            <FlatPriceEditor
              ...
              onPriceChange={onBasePriceChange}
            />
          </section>
        )}
      </div>
