        ...
        <div className={isEditing ? "detalle-producto-price-card-period-list" : ""} style={!isEditing ? { display: "flex", flexWrap: "wrap", gap: "0.4rem" } : undefined}>
          {periods.map((period, index) => (
            isEditing ? (
              ...
              <span key={period.id} style={{ backgroundColor: "var(--lds-logo-green-dark)", color: "white", padding: "4px 9px", fontSize: "0.72rem", borderRadius: "12px", fontWeight: "600", display: "inline-block" }}>
...
export default function ProductRestaurantPriceCard({
  ...
  return (
    <aside className={`detalle-producto-booking-card detalle-producto-price-card${isPricingActive ? " detalle-producto-price-card--editing" : ""}`}>
          ...
          {seasonalPricing?.high?.periods?.length > 0 ? (
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.28rem" }}>
              {seasonalPricing.high.periods.map((period) => (
                <span key={period.id} style={{ backgroundColor: "#ffffff", color: "var(--lds-logo-green-dark)", padding: "3px 6px", fontSize: "0.66rem", borderRadius: "12px", fontWeight: "800", display: "inline-block", textAlign: "center", boxShadow: "0 2px 4px rgba(0,0,0,0.15)" }}>
                  {formatSeasonPeriodLabel(period)}
                </span>
              ))}
            </div>
